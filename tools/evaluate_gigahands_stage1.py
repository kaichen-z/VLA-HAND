...
def load_config(path: Path) -> dict[str, Any]:
    from vitra.utils.config_utils import load_config as load_vitra_config

    return load_vitra_config(str(path))
...
def prepare_cognition_inputs(model, sample: dict[str, Any], current_state, current_state_mask, fov):
    ...
    instruction = str(sample["instruction"])
    text = instruction if instruction.startswith("<image>") else "<image>" + instruction
    model_inputs = model.processor(text=text, images=image, return_tensors="pt").to("cuda")
    return {
        ...
        "current_state_mask": current_state_mask.to(model_inputs["input_ids"].device),
        "current_state": current_state.to(model_inputs["input_ids"].device),
        "fov": fov.to(model_inputs["input_ids"].device) if fov is not None else None,
    }
...
def run_model_evaluation(args: argparse.Namespace) -> dict[str, Any]:
    ...
    teacher_config = load_config(args.teacher_config) if args.teacher_config is not None else config
    base_config = load_config(args.base_config_file) if args.base_config_file is not None else config
    checkpoint_config = load_config(args.checkpoint_config) if args.checkpoint_config is not None else config
    ...
    if args.teacher_checkpoint is not None or args.base_checkpoint is not None:
        if args.teacher_checkpoint is None or args.base_checkpoint is None:
            raise ValueError("--teacher_checkpoint and --base_checkpoint must be passed together")
        eval_indices = select_eval_indices(dataset, args)
        (output_dir / "data_ids.json").write_text(
            json.dumps({"data_ids": [int(idx) for idx in eval_indices]}, indent=2),
            encoding="utf-8",
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--teacher_label", default="base_vitra_teacher")
    parser.add_argument("--teacher_config", type=Path, default=None)
    parser.add_argument("--base_checkpoint", default=None)
    parser.add_argument("--base_label", default="base3b")
    parser.add_argument("--base_config_file", type=Path, default=None)
    parser.add_argument("--checkpoint_config", type=Path, default=None)
